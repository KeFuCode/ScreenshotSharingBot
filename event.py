...
class ActionReceiveEvent(Event):
    # message card receive event
    def __init__(self, dict_data, token):
...
class EventManager(object):
    ...
    event_type_map = dict()
    _event_list = [MessageReceiveEvent, UrlVerificationEvent, ActionReceiveEvent]
    ...
    def get_handler_with_event(token, encrypt_key):
        ...
        print(dict_data)
        encrypt_data = dict_data.get("encrypt")
        ...
        elif "om_" in dict_data.get("open_message_id", ""):
            event = ActionReceiveEvent(dict_data, token)
            return EventManager.event_callback_map.get(event.event_type()), event
